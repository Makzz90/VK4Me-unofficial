public class Class92 extends VKError {
   public Class242[] Field277;

   public final DataJson ProcessFields(JSONObject var1) {
      super.ProcessFields(var1);
      if (var1 == null) {
         return this;
      } else {
         JSONArray var4;
         if ((var1 = var1.Method180("response")) != null && (var4 = var1.optJSONArray("items")) != null) {
            this.Field277 = new Class242[var4.Count()];

            for(int var2 = 0; var2 < this.Field277.length; ++var2) {
               JSONObject var3;
               if ((var3 = var4.Method508(var2)) != null) {
                  this.Field277[var2] = (Class242)(new Class242()).ProcessFields(var3);
               }
            }
         }

         return this;
      }
   }
}
